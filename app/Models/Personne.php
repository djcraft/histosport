...
class Personne extends BaseModel
{
    ...
    /**
     * Recherche une personne normalisée selon les champs principaux.
     */
    public static function findNormalized($fields)
    {
        $query = self::query();
        foreach ([
            'nom', 'prenom', 'date_naissance', 'lieu_naissance_id'
        ] as $field) {
            $value = $fields[$field] ?? '';
    ...
    {
        $normalized = [];
        $keys = ['nom', 'prenom', 'date_naissance', 'lieu_naissance_id'];
